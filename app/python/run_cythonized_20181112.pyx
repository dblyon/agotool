...
cimport cython
# cython: language_level=3 # no effect
cimport numpy as np
...
cpdef count_terms_v4_cython(ENSP_2_funcEnumAssociations,
    ...
    """
    without returning 'funcEnum_count' the function does inplace change of 'funcEnum_count'
    :param funcEnum_associations: np.array (of variable length, with functional 
    associations as enumerations (instead of strings), 
    uint32, i.e. which functional associations are given for provided user input proteins)
    :param funcEnum_count: np.array (shape of array from 0 to max enumeration of 
    functional-terms, uint32, each position codes for 
    a specific functional term, the value is a count for the given user input)
    :return: None
    """
    cdef int N, i, k
...
cpdef calc_pvalues(unsigned int[::1] funcEnum_count_foreground,
                 ...
                 unsigned int foreground_n, unsigned int background_n,
                 double[::1] p_values,
                 cond_multitest):
    """
...
def map_funcEnum_2_ENSPs(protein_ans_list, ENSP_2_functionEnumArray_dict,
    ...
    :param foreground_ids_list_of_string: list of empty string, len of function_enumeration_len, list instead of array since len of longest string unknown and would take lots of memory
...
def run_genome_cy(taxid, protein_ans, background_n, preloaded_objects_per_analysis, static_preloaded_objects, args_dict, low_memory=False):
    if not low_memory:
    ...
    if not low_memory:
        ...
        ENSP_2_funcEnumAssociations = query.get_functionEnumArray_from_proteins(protein_ans)
    ...
    foreground_ids_arr_of_string = map_funcEnum_2_ENSPs(protein_ans, ENSP_2_functionEnumArray_dict, funcEnum_indices_for_foregroundIDs, foreground_ids_arr_of_string)# previously get_foreground_IDs_arr now map_funcEnum_2_ENSPs
...
def run_characterize_foreground_cy(protein_ans, preloaded_objects_per_analysis, static_preloaded_objects, args_dict, low_memory=False):
    ...
    if not low_memory:
        ...
        raise NotImplementedError
        # year_arr, hierlevel_arr, entitytype_arr, functionalterm_arr, indices_arr, category_arr,etype_2_minmax_funcEnum, function_enumeration_len, etype_cond_dict,taxid_2_proteome_count, taxid_2_tuple_funcEnum_index_2_associations_counts, lineage_dict_enum, blacklisted_terms_bool_arr,cond_etypes_with_ontology, cond_etypes_rem_foreground_ids = static_preloaded_objects
...
def run_compare_samples_cy(protein_ans_fg, protein_ans_bg, preloaded_objects_per_analysis, static_preloaded_objects, args_dict, low_memory=False):
    ...
    if not low_memory:
        ...
        ENSP_2_funcEnumAssociations = query.get_functionEnumArray_from_proteins(protein_ans_fg)
        count_terms_v4_cython(ENSP_2_funcEnumAssociations, funcEnum_count_foreground)
