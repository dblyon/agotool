...
def lineage_dict_direct_funcName_UPS_FIN(GO_obo, GO_obo_Jensenlab, UPK_obo, RCTM_hierarchy,
            DOID_obo_Jensenlab, BTO_obo_Jensenlab, direct_or_allParents, lineage_dict_direct_fn):
    if not os.path.exists(GO_obo):
        raise StopIteration
    if not os.path.exists(GO_obo_Jensenlab):
        raise StopIteration
    if not os.path.exists(UPK_obo):
        raise StopIteration
    if not os.path.exists(RCTM_hierarchy):
        raise StopIteration
    if not os.path.exists(DOID_obo_Jensenlab):
        raise StopIteration
    if not os.path.exists(BTO_obo_Jensenlab):
        raise StopIteration
