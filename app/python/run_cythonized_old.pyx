# cython: language_level=3, nonecheck=True, boundscheck=False, wraparound=False, profile=True
...
@boundscheck(False)
@wraparound(False)
cpdef count_terms_v4_cython(ENSP_2_funcEnumAssociations,
                          unsigned int[::1] funcEnum_count):
    """
    without returning 'funcEnum_count' the function does inplace change of 'funcEnum_count'
    :return: None
    """
    cdef int N, i, k
    for ENSP_funcEnums in ENSP_2_funcEnumAssociations:
        ENSP, funcEnum_associations = ENSP_funcEnums
        N = len(funcEnum_associations)
        for i in range(N):
            k = funcEnum_associations[i]
            funcEnum_count[k] += 1
...
@boundscheck(False)
@wraparound(False)
cpdef calc_pvalues(unsigned int[::1] funcEnum_count_foreground, unsigned int[::1] funcEnum_count_background,
        unsigned int foreground_n, unsigned int background_n, double[::1] p_values, cond_multitest):
    """
...
cdef BenjaminiHochberg_cy(double[::1] p_values,
    ...
    cdef:
        ...
        unsigned int enum_counter
        unsigned int index_2_BH

    for enum_counter, index_2_BH in enumerate(indices_2_BH, 1):
...
@wraparound(False)
@cdivision(True)
cdef BenjaminiHochberg_cy_v2(double[::1] p_values,
...
def map_funcEnum_2_ENSPs(protein_ans_list, ENSP_2_functionEnumArray_dict,
    ...
    for funcEnum, ENSPs in funcEnum_2_ENSPs_dict.items():
        foreground_ids_arr_of_string[funcEnum] = ";".join(sorted(ENSPs)) # needs to be sorted otherwise grouping will fail
...
def get_preloaded_objects_for_single_analysis(blacklisted_terms_bool_arr, function_enumeration_len=6834675, method="genome"):
    """
    funcEnum_count_foreground, funcEnum_count_background, p_values, p_values_corrected, cond_multitest, blacklisted_terms_bool_arr_temp, cond_terms_reduced_with_ontology, foreground_ids_arr_of_string, cond_filter, cond_PMIDs, cond_terms_reduced_with_ontology, cond_etypes_rem_foreground_ids_filtered
    """
    ...
    if method == "genome":
        ...
        cond_etypes_rem_foreground_ids_filtered = np.zeros(function_enumeration_len, dtype=bool)
        # 12
        return funcEnum_count_foreground, funcEnum_count_background, p_values, p_values_corrected, cond_multitest, blacklisted_terms_bool_arr_temp, cond_terms_reduced_with_ontology, foreground_ids_arr_of_string, cond_filter, cond_PMIDs, cond_terms_reduced_with_ontology, cond_etypes_rem_foreground_ids_filtered #, cond_multitest_parallel
        ...
        # 13
        return background_ids_arr_of_string, funcEnum_count_foreground, funcEnum_count_background, p_values, p_values_corrected, cond_multitest, blacklisted_terms_bool_arr_temp, cond_terms_reduced_with_ontology, foreground_ids_arr_of_string, cond_filter, cond_PMIDs, cond_terms_reduced_with_ontology, cond_etypes_rem_foreground_ids_filtered
    else:
        raise NotImplementedError

def filter_parents_if_same_foreground_cy(uint8[::1] blacklisted_terms_bool_arr_temp,
                                         cond_terms_reduced_with_ontology,
                                         dict lineage_dict_enum, df):
    cdef:
        unsigned int term_enum, lineage_term

    for _, group in df.groupby("foreground_ids", sort=False):
        group_terms = group.sort_values("hierarchical_level", ascending=False)["term_enum"].values
        for term_enum in group_terms:
            if blacklisted_terms_bool_arr_temp[term_enum] == 0: # False
                ...
                for lineage_term in lineage_dict_enum[term_enum] & set(group_terms):# bitwise intersection
                    blacklisted_terms_bool_arr_temp[lineage_term] = 1 # True

@boundscheck(False)
@wraparound(False)
@nonecheck(False)
def filter_parents_if_same_foreground_cy_v5(uint8[::1] blacklisted_terms_bool_arr_temp, #
                                             cond_terms_reduced_with_ontology,
                                             dict lineage_dict_enum,
                                             foreground_ids_arr,
                                             term_enum_arr,
                                             long[::1] indices_sort_order): #
    cdef:
        unsigned int term_enum, lineage_term
        unsigned int[::] group_terms
        int i, index_, N = indices_sort_order.shape[0]
        set lineage_set
        int max_ = blacklisted_terms_bool_arr_temp.shape[0]
        int max_2 = cond_terms_reduced_with_ontology.shape[0]

    bool_group_arr = np.zeros(term_enum_arr.shape[0], bool)
    previous_fg_id = foreground_ids_arr[indices_sort_order[0]]
    # for index_ in indices_arr:
    for i in range(N):
        index_ = indices_sort_order[i]
        fg_id = foreground_ids_arr[index_]
        # go through the array of sorted indices collecting the index positions of the same foreground_ids-group
        # at the end (when reaching the next group) iterate of the group of enum_terms and set blacklisted terms etc.
        if fg_id == previous_fg_id:
            bool_group_arr[index_] = True
            previous_fg_id = fg_id
        else:
            group_terms = term_enum_arr[bool_group_arr]
            for term_enum in group_terms:
                if blacklisted_terms_bool_arr_temp[term_enum] == 0: # False
                    cond_terms_reduced_with_ontology[term_enum] = True
                    lineage_terms_set = lineage_dict_enum[term_enum]
                    for lineage_term in lineage_terms_set:
                        if lineage_term in group_terms:
                            blacklisted_terms_bool_arr_temp[lineage_term] = 1 # True
            bool_group_arr[:] = False
            bool_group_arr[index_] = True
            previous_fg_id = fg_id
...
def run_genome_cy(taxid, protein_ans, background_n, preloaded_objects_per_analysis, static_preloaded_objects, args_dict, low_memory=False, filter_parents_cy=False, stuff=False):
    if not low_memory:
        ...
        year_arr, hierlevel_arr, entitytype_arr, functionalterm_arr, indices_arr, etype_2_minmax_funcEnum, function_enumeration_len, etype_cond_dict, taxid_2_proteome_count, taxid_2_funcEnum_index_2_associations, lineage_dict_enum, blacklisted_terms_bool_arr, cond_etypes_with_ontology, cond_etypes_rem_foreground_ids = static_preloaded_objects
    ...
    # cond_terms_reduced_with_ontology, cond_etypes_rem_foreground_ids_filtered = preloaded_objects_per_analysis
    ...
    ## count background
    if not low_memory:
        funcEnum_index_2_associations = taxid_2_funcEnum_index_2_associations[taxid]
    else:
    ...
    cond_foregroundIDs_2_query = cond_PMIDs | cond_etypes_with_ontology_filtered | cond_etypes_rem_foreground_ids_filtered
    ...
    # previously get_foreground_IDs_arr now map_funcEnum_2_ENSPs
    if not low_memory:
        ...
        foreground_ids_arr_of_string = ENSP_2_funcEnumAssociations # ToDo
    ...
    if filter_parents: # only for etypes with ontology, but since foreground IDs needed get them for all
        ...
        # return blacklisted_terms_bool_arr_temp, cond_terms_reduced_with_ontology, lineage_dict_enum, cond_etypes_with_ontology_filtered, term_enum_arr, foreground_ids_arr, df_with_ontology
        ### modifies cond_terms_reduced_with_ontology inplace
        if filter_parents_cy: ## bool_group_arr = np.zeros(term_enum_arr.shape[0], dtype=np.dtype("uint8")) # ToDo add to preload per analysis
            term_enum_arr = indices_arr[cond_etypes_with_ontology_filtered].view()
            foreground_ids_arr = foreground_ids_arr_of_string[cond_etypes_with_ontology_filtered].view()
            indices_sort_order = np.lexsort([hierlevel_arr[cond_etypes_with_ontology_filtered].view()[::-1], foreground_ids_arr_of_string[cond_etypes_with_ontology_filtered].view()])
            if stuff:
                return blacklisted_terms_bool_arr_temp, cond_terms_reduced_with_ontology, lineage_dict_enum, foreground_ids_arr, term_enum_arr, indices_sort_order
            filter_parents_if_same_foreground_cy_v5(blacklisted_terms_bool_arr_temp, cond_terms_reduced_with_ontology,
                                                    lineage_dict_enum, foreground_ids_arr,
                                                    term_enum_arr, indices_sort_order)
        else:
            ...
            filter_parents_if_same_foreground_cy(blacklisted_terms_bool_arr_temp, cond_terms_reduced_with_ontology, lineage_dict_enum, df_with_ontology)
...
def run_compare_samples_cy(protein_ans_fg, protein_ans_bg, preloaded_objects_per_analysis, static_preloaded_objects, args_dict, low_memory=False):
    ...
    if filter_parents: # only for etypes with ontology, but since foreground IDs needed get them for all
        ### modifies cond_terms_reduced_with_ontology inplace
        filter_parents_if_same_foreground_cy(blacklisted_terms_bool_arr_temp,
                                         cond_terms_reduced_with_ontology,
                                         lineage_dict_enum, df_with_ontology)
    else: # since no filtering done use all etypes with ontology
        cond_terms_reduced_with_ontology = cond_filter & cond_etypes_with_ontology
