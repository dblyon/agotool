...
df = pd.DataFrame.from_dict({'term': {0: 'GOCC:0043229', 1: 'GOCC:0098588', 2: 'GOCC:0005730', 3: 'GO:0005730', 4: 'GO:0005783', 5: 'GO:0031410', 6: 'KW-0732', 7: 'KW-0156', 8: 'KW-0010'}, 'description': {0: 'Intracellular organelle', 1: 'Bounding membrane of organelle', 2: 'Nucleolus', 3: 'nucleolus', 4: 'endoplasmic reticulum', 5: 'cytoplasmic vesicle', 6: 'Signal', 7: 'Chromatin regulator', 8: 'Activator'}, 'FG_count': {0: 37, 1: 92, 2: 126, 3: 131, 4: 63, 5: 23, 6: 9, 7: 41, 8: 50}, 'logFDR': {0: 3, 1: 4, 2: 5, 3: 6, 4: 7, 5: 8, 6: 5, 7: 1, 8: 2}, 'effectSize': {0: 0.053, 1: -0.049, 2: 0.046, 3: 0.047, 4: -0.040, 5: -0.027, 6: -0.024, 7: 0.025, 8: 0.025}, 'category': {0: 'TM', 1: 'TM', 2: 'TM', 3: 'GOCC', 4: 'GOCC', 5: 'UPK', 6: 'UPK', 7: 'GOCC', 8: 'UPK'}})
...
def update_table_style(selectedData):
    ...
    """
    # print("selectedData {}".format(selectedData))
    # import pdb
    # pdb.set_trace()
    if selectedData is not None:
        for key, values in selectedData.items():
            print(key)
            for val in values:
                print(val)
        print()
        for point in selectedData['points']:
            print(point['pointIndex'])
        print()

        selected_styles = [{'if': {'row_index': point['pointIndex']},
                            'backgroundColor': 'gold'} for point in selectedData['points']]
        for point in selectedData['points']:
            term = point["customdata"][0]
            # --> "if": {"column_id": "term"} with "term" == term --> then highlight

        table_style_conditions = (selected_styles + [{'if': {'row_index': 'odd'}, 'backgroundColor': "#F5F5F5", }] + [{"if": {"state": "selected"}, "backgroundColor": "inherit !important", "border": "inherit !important", "text_align": "inherit !important", }] + [{"if": {"state": "active"}, "backgroundColor": "inherit !important", "border": "inherit !important", "text_align": "inherit !important", }])
    else:
